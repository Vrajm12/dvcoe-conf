...
export default async function Gallery2021() {
  ...
  const hasImages = (cmsContent?.event_images?.length ?? 0) > 0 || (cmsContent?.hosting_campus_images?.length ?? 0) > 0;

  return (
    <main className="py-16 bg-gray-50 min-h-screen">
      <div className="container mx-auto px-4">
        <h1 className="text-4xl font-bold text-center mb-4 text-gray-800">
          {cmsContent?.page_title || 'Gallery 2021'}
        </h1>
        <p className="text-center text-gray-600 mb-12">
          {cmsContent?.page_intro || 'Highlights from ICCET-2021 Conference'}
        </p>
          ...
          {!hasImages ? (
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-lg p-12 text-center">
              ...
              <p className="text-gray-600 mb-4">
                We&apos;re curating the best moments from ICCET-2021. Gallery images will be uploaded soon.
              </p>
