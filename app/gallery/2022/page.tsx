...
export default async function Gallery2022() {
  ...
  const hasImages = (cmsContent?.event_images?.length ?? 0) > 0 || (cmsContent?.news_images?.length ?? 0) > 0;

  return (
    <main className="py-16 bg-gray-50 min-h-screen">
      <div className="container mx-auto px-4">
        <h1 className="text-4xl font-bold text-center mb-4 text-gray-800">
          {cmsContent?.page_title || 'Gallery 2022'}
        </h1>
        <p className="text-center text-gray-600 mb-12">
          {cmsContent?.page_intro || 'Highlights from ICCET-2022 Conference'}
        </p>
          ...
          {!hasImages ? (
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-lg p-12 text-center">
              ...
              <p className="text-gray-600 mb-4">
                We&apos;re curating the best moments from ICCET-2022. Gallery images will be uploaded soon.
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {cmsContent?.news_images?.map((src, idx) => (
                <div key={`news-${idx}`} className="bg-white rounded-lg shadow-lg overflow-hidden">
                  <img src={src} alt={`News ${idx + 1}`} className="w-full h-64 object-cover" />
                  <div className="p-4">
                    <h3 className="font-semibold text-gray-800">News Photo {idx + 1}</h3>
                  </div>
                </div>
              ))}
            </div>
